...
```{r playlist information, echo=FALSE, results='hide',message=FALSE}
...
```{r combine, echo=FALSE, results='hide',message=FALSE}
tracks <-
  bind_rows(
...
```{r plot 2, echo=FALSE, results='hide',message=FALSE}
...
```{r calculate energy differences, echo=FALSE, results='hide',message=FALSE}
...
```{r plot 3 differences, echo=FALSE, results='hide',message=FALSE}
most_different <- tracks %>% top_n(20, energy_absdiff) # get 10 biggest differences (remix and original have same difference value)
ggplot(most_different, aes(x=category,y=energy,fill=as.factor(id))) + 
geom_point(aes(size=tempo), alpha=0.3, shape=21) +
